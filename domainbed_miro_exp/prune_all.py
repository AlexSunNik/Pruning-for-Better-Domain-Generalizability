...
from domainbed.prune_trainer import compute_score, prune_finetune, prune_finetune_nonuni
import pickle
...
def main():
    ...
    parser.add_argument("--testenv", type=int, default=0, help="Which test environment to use")
    ...
    logger = Logger.get(args.out_dir / f"log_prune_testenv{testenv}.txt")
    if args.debug:
    ...
    logger.info(f"Prune and finetune for test environment {testenv}")
    dataset_name = args.dataset
    print(dataset_name)
    for idx, test_env in enumerate(args.test_envs):
        if idx != testenv:
            continue
        score_save_name = f"{dataset_name}_{testenv}_score.pkl"
        if os.path.isfile(score_save_name):
            with open(score_save_name, 'rb') as f:
                scores = pickle.load(f)
        else:
            scores, scores1 = compute_score(
                test_env,
                args=args,
                hparams=hparams,
                n_steps=n_steps,
        ...
        elif prune_method == "nonuni":
            res, records = prune_finetune_nonuni(
                scores,
                test_env,
                args=args,
                hparams=hparams,
