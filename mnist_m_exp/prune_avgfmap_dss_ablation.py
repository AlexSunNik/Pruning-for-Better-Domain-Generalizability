...
exp_name = f"reverseDSS_prunetimes{PRUNE_TIMES}_ratio{total_pruneratio}_epochs{epochs}_{date_time}"
...
for prune_iter in range(PRUNE_TIMES):
    ...
    avg_src_fmaps = defaultdict(list)
    avg_tgt_fmaps = defaultdict(list)
    # Get some data
    # *************************************************************************************** #
    for batch_idx, (source_data, target_data) in enumerate(zip(source_test_loader, target_test_loader)):
        ...
        for name, fmap in source_fmaps.items():
            ...
            avg_src_fmaps[name].append(fmap)
            avg_tgt_fmaps[name].append(target_fmap)
    # *************************************************************************************** #
    avg_src_fmaps = {k:torch.mean(torch.cat(v, dim=0), dim=0) for k,v in avg_src_fmaps.items()}
    avg_tgt_fmaps = {k:torch.mean(torch.cat(v, dim=0), dim=0) for k,v in avg_tgt_fmaps.items()}
    # Get scores
    # Compute Score
    scores = {}
    scores1 = {}
    ...
    for name, fmap in source_fmaps.items():
        # print(name)
        val, idxs = torch.sort(torch.nan_to_num(torch.tensor(scores[name]), nan=-10), descending=True)
