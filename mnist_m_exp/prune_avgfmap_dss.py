...
exp_name = f"prunetimes{PRUNE_TIMES}_ratio{total_pruneratio}_epochs{epochs}_{date_time}"
...
for prune_iter in range(PRUNE_TIMES):
    ...
    for name, fmap in source_fmaps.items():
        # print(name)
        val, idxs = torch.sort(torch.nan_to_num(torch.tensor(scores[name]), nan=-10))
